...
\begin{homeworkProblem}{ Short on Average.}
  Given a directed graph $G =
    ...
    \item Show how to use
    Bellman-Ford-Moore to find a cycle of zero weight assuming there
    is no negative-weight cycle. (Hint: Recall how you used B-F to
    compute feasible potentials in Johnson’s algorithm from Lecture
    \#4.)
  \item
...
\begin{enumerate}[label=\alph*]
\item For two edge sets $E, E0'$ with spanning forests $F, F'$
...
\item\emph{Large-girth graphs are Sparse.} On a seemingly different
  ...
  \begin{enumerate}
    \item  (Do
  not submit.) Show that if we set $\alpha = n − 1$, then you will get
  Kruskal’s algorithm. Also, observe that by construction, the graph $H$
  at the end of the process is an $(n − 1)$-stretch spanner. (In fact,
  an $(n − 1)$-stretch spanning tree.)
